...
class S3DataSync:
    def __init__(self, s3_sync_config: S3StorageConfig, dest_dir: str):
        """
        ...
        Args:
            ...
            dest_dir: existing local directory to keep in sync
        """
        self.dest_dir = Path(dest_dir).resolve()
        if not self.dest_dir.is_dir():
            raise ValueError(f"dest_dir does not exist or is not a directory: {dest_dir}")
        ...
        self.suffix = s3_sync_config.path_suffix
        self.prefix = path_parts[1].lstrip("/") if len(path_parts) > 1 else ""
    ...
    def _local_path_for_object(self, obj_name: str) -> Path:
        item_path = obj_name[len(self.prefix):] if self.prefix else obj_name
        return self.dest_dir / item_path
    ...
    def _remove_file(self, file_path: Path) -> None:
        ...
        while parent != self.dest_dir and not any(parent.iterdir()):
            parent.rmdir()
    ...
    def sync(self) -> None:
        _log.info(
            "Starting full sync from bucket=%s prefix=%s",
            self.bucket,
            self.prefix or "(root)",
        )
        ...
        for obj in objects:
            ...
            if needs_download:
                _log.info("Downloading %s -> %s", obj.object_name, local_path)
    ...
    def observe(self, stop_signal: threading.Event) -> None:
        ...
        _log.info(
            "Starting S3 event observer for bucket=[%s] prefix=[%s]",
            self.bucket,
            self.prefix or "(root)",
        )
                    ...
                    for event in events:
                        ...
                        for record in event.get("Records", []):
                            ...
                            if event_name.startswith("s3:ObjectCreated"):
                                _log.info("S3 created/updated: %s", obj)
                                local_path.parent.mkdir(parents=True, exist_ok=True)
                                self.client.fget_object(
                                    self.bucket,
                                    obj,
