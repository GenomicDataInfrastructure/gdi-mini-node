...
from .data.data import BeaconData
...
def beacon_data(data: BeaconData, title: str) -> str:
...
"""
    ...
    content += beacon_data(DATA.aggregated_beacon, "Aggregated")
    content += beacon_data(DATA.sensitive_beacon, "Sensitive")
...
def get_health(_: Request) -> JSONResponse:
    ...
    return JSONResponse({"timestamp": ts, "version": app_version, "healthy": True})


def check_access(request: Request) -> PlainTextResponse | None:
...
def file_sort_key(chr_group: str) -> int:
    middle = chr_group.index(".")
    key = int(chr_group[middle + 1:])
    chromosome = chr_group[0:middle]
    if chromosome == "X":
        return key + 25000
    elif chromosome == "Y":
        return key + 26000
